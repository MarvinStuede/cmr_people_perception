/*!
  *\file: object3d_detector.cpp
  * \author: Alexander Petersen
  * \date: 21.11.19
  * \brief: Object3dDetector Classe:
  * Class to predict People in 3D Pointcloud
  */
...
private:
  ...
  bool new_msgs=false;
  spencer_tracking_msgs::DetectedPersons detected_persons_global;
 ...
 */
  ...
  /// Timercallback for publishing predictions
...
Object3dDetector::Object3dDetector() {
  ...
  // Init subcriber
  point_cloud_sub_ = node_handle_.subscribe<sensor_msgs::PointCloud2>("/velodyne_points_throttle", 1, &Object3dDetector::pointCloudCallback, this);
  ...
  spencer_pub= private_nh.advertise<spencer_tracking_msgs::DetectedPersons>("/spencer/perception_internal/detected_persons/velodyne2", 100);
  spencer_odom_pub=private_nh.advertise<sg_msgs::DetectedPersonsOdom>("/spencer/perception_internal/detected_persons/pre/velodyne",100);
  my_timer_ = private_nh.createTimer(ros::Duration(0.066), &Object3dDetector::timerCallback, this);


  /*** Parameters ***/
  private_nh.param<bool>("print_fps", print_fps_, false);
  private_nh.param<std::string>("frame_id", frame_id_, "velodyne");
  private_nh.param<float>("z_limit_min", z_limit_min_, -1.2);
...
void Object3dDetector::timerCallback(const ros::TimerEvent &evt)
{

  // checks for new detections. if not, an empty vector gets out
  if(new_msgs)
  {
    tf::StampedTransform pose_tf_trans;
    try
    {

      listener_.lookupTransform("velodyne","odom",ros::Time(0),pose_tf_trans);
...
//  detected_persons_global.header.stamp =time_;
//  detected_persons_global.header.frame_id = frame_id_;
    for (auto &detection:detected_persons_global.detections) {
      tf::Transform pose_tf;
      tf::poseMsgToTF(detection.pose.pose,pose_tf);
      tf::poseTFToMsg(pose_tf_trans*pose_tf_trans1_*pose_tf,detection.pose.pose);
    }

  detected_persons_global.header.stamp=ros::Time::now();
  spencer_pub.publish(detected_persons_global);
 new_msgs=false;
  }
  else {

    detected_persons_global.detections.clear();
    detected_persons_global.header.stamp = ros::Time::now();
    detected_persons_global.header.frame_id = frame_id_;

//    for(auto detections:detected_persons_global.detections)
//    {
//    detections.detection_id = current_detection_id;
...
void Object3dDetector::pointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr& ros_pc2) {
 ...
 try
  ...
  {
    ROS_ERROR("Lookup Exception: %s",e.what());
  }
  catch(tf::ConnectivityException e)
  {
    ROS_ERROR("Connectivity Exception: %s",e.what());
  }
  catch(tf::ExtrapolationException e)
  {
    ROS_ERROR("Extrapolation Exception: %s",e.what());
  }
  time_=ros_pc2->header.stamp;
  time2_=ros::Time::now();
  if(print_fps_)if(reset){frames=0;start_time=clock();reset=false;}//fps
  sensor_msgs::PointCloud2 ros_pc3;

  pcl_ros::transformPointCloud("velodyne",*ros_pc2,ros_pc3,listener_);


  pcl::PointCloud<pcl::PointXYZI>::Ptr pcl_pc(new pcl::PointCloud<pcl::PointXYZI>);
  pcl::fromROSMsg(ros_pc3, *pcl_pc); // transforms pointcloud msgs to pcl format
