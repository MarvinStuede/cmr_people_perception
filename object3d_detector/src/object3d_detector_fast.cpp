...
*/
/*!
  *\file: object3d_detector_fast.cpp
  * \author: Alexander Petersen
  * \date: 21.11.19
  * \brief: Object3dDetector fast Classe:
  * Class to predict People in 3D Pointcloud with depth_clustering package
  */
...
//#include <pcl/visualization/cloud_viewer.h> //COMMENTED DUE TO COMPILER ERROR (10/2020)
// SVM
#include "svm.h"

#include <spencer_tracking_msgs/DetectedPersons.h>

#include <sg_msgs/PointCloudArray.h>
...
private:
  ...
  ros::Publisher spencer_odom_pub;
  ...
  std::string model_file_name_;   /**< Filename for SVM Model */
  std::string range_file_name_;   /**< Filename for SVM Range */
  struct svm_node *svm_node_;     /**< SVM Node -> current extract features from pointcloud */
  struct svm_model *svm_model_;   /**< SVM Model */
  bool use_svm_model_;            /**< Control if svm was loaded */
  bool is_probability_model_;     /**< Control if failed SVM */
  float svm_scale_range_[FEATURE_SIZE][2];  /**< Array to rescale features */
  float x_lower_;   /**< lower limit for rescaling */
  float x_upper_;   /**< upper limit for rescaling */
  float human_probability_;  /**< Probability for prediction */
  bool human_size_limit_;   /**< bool if cluster are pre-filtert */
  ...
  int detection_id_increment, detection_id_offset, current_detection_id;
    ...
    sg_msgs::DetectedPersonsOdom detected_persons_odom_global;
    tf::TransformListener listener_;
    tf::StampedTransform pose_tf_trans1_;

public:
  /** Creating Object3dDetector Class
   * Initialising all subscribers, publishers and parameters
 */
  Object3dDetector();
  ~Object3dDetector();

   /// Callback for recieving Clusters from depth_clustering
  void pointCloudCallback(const sg_msgs::PointCloudArray::ConstPtr& ros_pc2);
  ...
  /** \brief Extract cluster.
    ...
    */
  void extractCluster(std::list< pcl::PointCloud<pcl::PointXYZ>::Ptr> point_list);
  ...
  /** \brief Extract features.
    ...
    */
  void extractFeature(pcl::PointCloud<pcl::PointXYZ>::Ptr pc, Feature &f,
  ...
  /** \brief Classify.
    *
    * This method is predicting, if the given Clusters are people or not
    */
  void classify();
};
...
Object3dDetector::Object3dDetector() {
  ...
  // Init subcriber
  point_cloud_sub_ = node_handle_.subscribe<sg_msgs::PointCloudArray>("/cluster_velodyne_points", 1, &Object3dDetector::pointCloudCallback, this);
  ...
  spencer_pub= private_nh.advertise<spencer_tracking_msgs::DetectedPersons>("/spencer/perception_internal/detected_persons/velodyne_fast", 100);

  spencer_odom_pub=private_nh.advertise<sg_msgs::DetectedPersonsOdom>("/spencer/perception_internal/detected_persons/pre/velodyne_fast",100);
  ...
  private_nh.param<double>("pose_variance", pose_variance, 0.05);
  private_nh.param("detection_id_offset",    detection_id_offset, 13);
  private_nh.param("detection_id_increment", detection_id_increment, 1);
  ...
  private_nh.param<std::string>("range_file_name", range_file_name_, "");
   ...
   //loading SVM model and allocats memory
...
void Object3dDetector::pointCloudCallback(const sg_msgs::PointCloudArray::ConstPtr& ros_pc2) {
  if(print_fps_)if(reset){frames=0;start_time=clock();reset=false;}//fps

  try
   {
   listener_.lookupTransform("odom","velodyne",ros::Time(0),pose_tf_trans1_);
   }
   catch(ros::Exception e)
   {
     ROS_ERROR("In erster Transformation: %s",e.what());
   }
   std::list< pcl::PointCloud<pcl::PointXYZ>::Ptr> point_list;
           //pcl::PointCloud<pcl::PointXYZ>::Ptr[] bla(new pcl::PointCloud<pcl::PointXYZ>);

     for(const auto points : ros_pc2->pointcloud_array )
     {
         pcl::PointCloud<pcl::PointXYZ>::Ptr pcl_pc(new pcl::PointCloud<pcl::PointXYZ>);
         pcl::fromROSMsg(points, *pcl_pc); // transforms pointcloud msgs to pcl format
         point_list.push_back(pcl_pc);
     }


  extractCluster(point_list);
...
void Object3dDetector::extractCluster(std::list< pcl::PointCloud<pcl::PointXYZ>::Ptr> point_list) {
  features_.clear(); //resets Features
  bool stop=false;  //debugging
  human_size_limit_=true;  //checking if cluster has human size

  // iterates every cluster
  for(auto &pc : point_list)
{


    // checking min. size of cluster
    if(pc->size() > cluster_size_min_) {

          // view pointcloud for debugging
          if(stop){
 		//COMMENTED DUE TO COMPILER ERROR ON ROS MELODIC (10/2020)
            ...
            max[2]-min[2] < 0.5 || max[2]-min[2] > 2.0))
        {
            ...
            Feature f;
            extractFeature(pc, f, min, max, centroid);
...
void computeSlice(pcl::PointCloud<pcl::PointXYZ>::Ptr pc, int n, float *slice) {
...
void computeIntensity(pcl::PointCloud<pcl::PointXYZ>::Ptr pc, int bins, float *intensity) {
    /*
...
void Object3dDetector::extractFeature(pcl::PointCloud<pcl::PointXYZ>::Ptr pc, Feature &f,
				      Eigen::Vector4f &min, Eigen::Vector4f &max, Eigen::Vector4f &centroid) {
  f.centroid = centroid;
  f.min = min;
  f.max = max;
...
void Object3dDetector::classify() {
  ...
  visualization_msgs::MarkerArray marker_array; // visualisation bounding box of person
  geometry_msgs::PoseArray pose_array;          // vis. position of person
  spencer_tracking_msgs::DetectedPersons detected_persons;

   // goes to every featurevector and predicts cluster
  for(std::vector<Feature>::iterator it = features_.begin(); it != features_.end(); ++it) {
    if(use_svm_model_) {
      ...
      //std::cerr  << ", number_points = " << it->number_points << ", min_distance = " << it->min_distance <<"x: "<<it->centroid[0]<<"y: "<<it->centroid[1]<<"z: "<<it->centroid[2]<< std::endl;
	...
	double prob_estimates[svm_model_->nr_class];
        ...
        //std::cerr<<"prob "<<prob_estimates[0]<<std::endl;
        //std::cerr<<"prob size: "<<svm_model_->nr_class<<std::endl;
	if(prob_estimates[0] < human_probability_)
	  continue;
   //std::cerr<<"prob fast: "<<prob_estimates[0]<<std::endl;
      } else {
    ...
    spencer_tracking_msgs::DetectedPerson detected_person;
        detected_person.modality = spencer_tracking_msgs::DetectedPerson::MODALITY_GENERIC_LASER_3D;
        detected_person.confidence = 1; // FIXME: normalize
  ...
  if(pose_array.poses.size()) {
    ...
    detected_persons.header.frame_id = frame_id_;
    spencer_pub.publish(detected_persons);

    detected_persons_odom_global.DetectedPersons=detected_persons;
    geometry_msgs::TransformStamped transform_geo;;
    tf::transformStampedTFToMsg(pose_tf_trans1_,transform_geo);
    detected_persons_odom_global.Transform=transform_geo;
    spencer_odom_pub.publish(detected_persons_odom_global);
  }
}

int main(int argc, char **argv) {
  ros::init(argc, argv, "object3d_detector_fast");
  Object3dDetector d;
  ros::spin();
