...
*/
/*!
  *\file: training_ob_multi.cpp
  * \author: Alexander Petersen
  * \date: 21.11.19
  * \brief: TrainingOBMulti Classe:
  * Class to train SVM with 6 features
  */
...
void TrainingObMulti::load_data()
{
    ...
    /*** save data to file ***/
     std::ofstream s;
     s.open("svm_training_data_test_multi");
    ...
    /*** save data to file ***/
    std::fstream fs;
    fs.open ("svm_training_data_test_multi", std::fstream::in | std::fstream::out | std::fstream::app);
...
//     std::cerr << "svm scale range [attribute " << i << "]: " << svm_range_[i][0] << ", " << svm_range_[i][1] << std::endl;
  ...
  /*** debug saving ***/
if(svm_save_model("pedestrian_test_multi_new.model", svm_model_) == 0)
   std::cerr << "A model has been generated here: ~/.ros/pedestrian.model" << std::endl;
}

int main(int argc, char **argv) {
  ros::init(argc, argv, "traning_ob_multi");
  TrainingObMulti d;
  /*** single-thread ***/
  ros::spin();
  /*** multi-thread ***/
